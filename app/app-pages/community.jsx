...
const CommunityPage = () => {
  ...
  const loadChatsFromStorage = async () => {
    try {
      ...
      if (savedConversations) {
        ...
        console.log(
          "✅ Loaded chats from storage:",
          cleanedConversations.length,
        );
      } else {
        setChats([]);
    ...
    try {
      ...
      if (cachedUsers) {
        try {
          ...
          console.log(
            "✅ Loaded users from cache (fallback):",
            parsedUsers.length,
          );
        } catch (e) {
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (currentUser) {
          ...
          const updatedAvatarsMap = {
            ...userAvatarsMap,
            [currentUser.id]: asset.uri,
          };
  ...
  const renderRightActions = (progress, dragX, onDelete) => {
    ...
    return (
      ...
      >
        <Animated.View
          style={{ transform: [{ scale: trans }], alignItems: "center" }}
        >
  ...
  const handleOpenGroupChat = (group) => {
    try {
      ...
      router.push(
        `/app-pages/group-chat?groupId=${gid}&groupName=${gname}&avatar=${gav}&members=${encodedMembers}`,
      );
    } catch (e) {
  ...
  const pickGroupAvatar = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Required",
          "Please grant camera roll permissions to upload group image.",
        );
        return;
      }
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Success",
        `Group "${newGroupName.trim()}" created with ${allMembers.length} members!`,
      );
    } catch (error) {
  ...
  const renderChatItem = ({ item }) => {
    const displayName =
      userNamesMap[item.userId] || item.userName || "Traveler";
    ...
    return (
      <Swipeable
        renderRightActions={(progress, dragX) =>
          renderRightActions(progress, dragX, () =>
            deleteChat(item.id, item.userId),
          )
        }
  ...
  const filteredChats = chats.filter((chat) => {
    const displayName =
      userNamesMap[chat.userId] || chat.userName || "Traveler";
    return displayName?.toLowerCase().includes(searchQuery.toLowerCase());
  });
  ...
  const availableUsersForGroup = allUsersList.filter(
    (user) => user.id !== currentUser?.id && !selectedUsers.includes(user.id),
  );
                ...
                <View style={styles.userProfileHeader}>
                  ...
                  >
                    {userProfileImage ? (
                      <Image
                        source={{ uri: userProfileImage }}
                        style={styles.userProfileHeaderImage}
                      />
                    ) : (
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.inputLabel}>
                Add Members ({availableUsersForGroup.length} available)
              </Text>
              <ScrollView
                  ...
                  availableUsersForGroup.map((user) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <Text style={styles.userSelectAvatar}>
                          {user.avatar || "👤"}
                        </Text>
                        <Text style={styles.userSelectName}>{displayName}</Text>
                        <Text
                          style={[
                            styles.userSelectAdd,
                            isSelected && styles.userSelectAdded,
                          ]}
                        >
