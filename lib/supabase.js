...
export const isInvalidRefreshTokenError = (error) => {
  ...
  const lowerMessage = (error.message || "").toString().toLowerCase()
  return (
...
export const clearStaleAuthSession = async (error) => {
  if (!isInvalidRefreshTokenError(error)) return

  console.warn("Supabase auth refresh failed due to stale refresh token; clearing session.")
  try {
    await supabase.auth.signOut()
  } catch (signOutError) {
    console.warn("Failed to sign out stale Supabase session:", signOutError)
  }
}
...
export const sendDirectMessage = async (senderId, receiverId, message) => {
  if (!isValidUuid(senderId) || !isValidUuid(receiverId)) {
    console.warn("Skipping direct message: invalid sender or receiver id", senderId, receiverId);
    return null;
  }
...
export const subscribeToDirectMessages = (userId, onNewMessage) => {
  if (!isValidUuid(userId)) {
    console.warn("Cannot subscribe to direct messages: invalid user id", userId);
    return null;
  }
